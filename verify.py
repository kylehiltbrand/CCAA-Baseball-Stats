...
checks = [
    ...
    ('league filter drives players',       "if(curTab==='players'){ buildTeamSelect(); renderPlayers(); }", s, 1),
    ...
    ('id pt-r',                            'id="pt-r"', s, 1),
    # --- filters ---
    ('class pill group',                   'setPClass(', s, 5),
    ('team select element',                'id="team-sel"', s, 1),
    ('setPTeam defined',                   'function setPTeam(', s, 1),
    ('setPClass defined',                  'function setPClass(', s, 1),
    ('buildTeamSelect defined',            'function buildTeamSelect(', s, 1),
    ('pFilter defined',                    'function pFilter(', s, 1),
    ('kind pills scoped',                  ".querySelectorAll('.subtab.kind')", s, 1),
    ('class pills scoped',                 ".querySelectorAll('.subtab.cls')", s, 1),
    ('no unscoped subtab clear',           ".querySelectorAll('.subtab').forEach", s, 0),
    ('filters applied to rows',            'const rows=pFilter(pPool(pKind));', s, 1),
    ('empty state block',                  'emptyst', s, 3),
    ('class note element',                 'id="cls-note"', s, 1),
    ('league change rebuilds teams',       "buildTeamSelect(); renderPlayers();", s, 2),
    ...
    ('team select css',                    '.tsel{', s, 2),
    ('control label css',                  '.ctllbl{', s, 2),
    ('empty state css',                    '.emptyst{', s, 1),
    ('control group css',                  '.ctlgrp{', s, 2),
    ('control groups in markup',           '<span class="ctlgrp">', s, 2),
    ('qrow css',                           '.qrow{', s, 3),
